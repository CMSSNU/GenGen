...
def cal_Mass(my_file, id=25):
    ...
    for evt in events:
        ...
        #print "idx="+str(evt.PARTICLE_LIST.index(ptl))+"pid="+str(ptl.IDUP)+" status="+str(ptl.ISTUP)                                                                                     
        particle_list+=check_Id(evt, id)

    #print MH_list
    #print "<MH>="+str(numpy.mean(MH_list))
    #print "d<MH>="+str(numpy.std(MH_list)/math.sqrt(len((MH_list))))
    dic={'Mptcl':numpy.mean(particle_list), \
         'dMptcl':numpy.std(particle_list)/sqrt(len((particle_list))) \
     }
...
if __name__=="__main__" :
    dirlist=glob.glob("test_ChargedHiggsToCB_M085*")
    ...
    for mydir in dirlist:
        ...
        Mass_cal=cal_Mass(mydir+"/cmsgrid_final.lhe", ID)
        ...
        print '@Mass_cal["Mass"]='+str(Mass)
        print '@Mass_cal["dMass"]='+str(dMass)
        #if abs(MH-hmass) > 2*sigma:
	#  print 'gen mass - seting mass > 2 sigma !!!!!!!!!!!!!!!!!!!!!!!!!!!!!', MH-hmass
	#print 'num of leptons',nlep
        #if not  nlep==ch:
        #    print "------------"
        #    print mydir
